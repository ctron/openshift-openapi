...
fn main() -> Result<(), Error> {
    ...
    let threads: Vec<_> = supported_version::ALL
        .iter()
...
fn run(
    ...
    let mut spec: swagger20::Spec = {
        ...
        log::info!("Loading spec file at {} ...", spec_url);
    ...
    log::info!(
        "OK. Spec has {} definitions and {} operations after filtering out non-OpenShift items",
        expected_num_generated_types,
        expected_num_generated_apis
    );
    ...
    for definition_path in spec.definitions.keys() {
        ...
        if !definition_path.starts_with("com.github.openshift.") {
        ...
        let run_result = k8s_openapi_codegen_common::run(
            ...
            |operation_optional_parameters_name, operation_result_name| {
                let mut parent_mod_rs_file_and_mod_name =
                    parent_mod_rs_file_and_mod_name.borrow_mut();
    ...
    {
        let mut mod_root_file = std::io::BufWriter::new(
            std::fs::OpenOptions::new()
                .append(true)
        ...
        for operation in spec.operations {
            ...
            {
                num_generated_apis += 1; // FIXME: count as generated for now
                continue;
                /*
                return Err(format!(
