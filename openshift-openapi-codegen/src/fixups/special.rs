...
pub(crate) fn create_delete_optional(
    ...
    let delete_options_properties =
        if let crate::swagger20::SchemaKind::Properties(properties) = &delete_options_schema.kind {
            ...
            return Err(
            "io.k8s.apimachinery.pkg.apis.meta.v1.DeleteOptions is not a SchemaKind::Properties"
                .into(),
        );
        };
        ...
        for operation in &mut spec.operations {
            ...
            if !operation.id.starts_with(description) {
                return Err(format!(
                    ...
                    operation.id, operation.kubernetes_action, description
                )
                .into());
            }

            {
                ...
                for expected_parameter_name in optional_parameters {
                    let expected_parameter = if let Some(expected_parameter) = operation
                        .parameters
                        .iter()
                        .find(|p| p.name == *expected_parameter_name && !p.required)
                    {
                        ...
                        return Err(format!(
                            ...
                            operation.id, description, expected_parameter_name
                        )
                        .into());
                    };
                    ...
                    optional_definition
                        .entry(crate::swagger20::PropertyName(
                            expected_parameter_name.to_owned(),
                        ))
    ...
    for operation in &mut spec.operations {
        ...
        for expected_parameter_name in list_optional_parameters {
                ...
                .find(|p| p.name == *expected_parameter_name && !p.required)
            {
                ...
                return Err(format!(
                    "operation {} is a list operation but doesn't have a {} parameter",
                    operation.id, expected_parameter_name
                )
                .into());
            };
...
pub(crate) fn list(spec: &mut crate::swagger20::Spec) -> Result<(), crate::Error> {
    ...
    for (definition_path, definition) in &spec.definitions {
        ...
        let item_schema = if let Some((
            crate::swagger20::Schema {
                kind:
                    crate::swagger20::SchemaKind::Ty(crate::swagger20::Type::Array {
                        items: item_schema,
                    }),
                ..
            },
        ...
        )) = properties.get(&items_property_name)
        {
            ...
            return Err(format!(
                "definition {} looks like a list but doesn't have an items property",
                definition_path
            )
            .into());
        };

        let item_ref_path = if let crate::swagger20::SchemaKind::Ref(item_ref_path) =
            &item_schema.kind
        {
            let item_schema =
            ...
            return Err(format!(
                "definition {} looks like a list but its items property is not a ref",
                definition_path
            )
            .into());
        };
        ...
        for operation in &mut spec.operations {
            if operation.kubernetes_action == Some(crate::swagger20::KubernetesAction::Post) {
                if let crate::swagger20::OperationResponses::Map(responses) = &operation.responses {
                    ...
                    // 1.8 and earlier did not have 201 and 202
                    {
        ...
        for operation in &mut spec.operations {
            match operation.kubernetes_action {
                Some(crate::swagger20::KubernetesAction::Delete) => {
                    if let crate::swagger20::OperationResponses::Map(responses) =
                        &operation.responses
                    {
                        let mut response_status_codes: Vec<_> = responses.keys().copied().collect();
                        response_status_codes.sort();
                        if response_status_codes
                            ...
                            && response_status_codes != [http::StatusCode::OK]
                        // 1.11 and earlier did not have 202
                        {
                        ...
                        let should_use_common_response_type =
                            responses
                                ...
                                .all(|crate::swagger20::Schema { kind, .. }| {
                                    ...
                                    if let Some(kubernetes_group_kind_version) =
                                        &operation.kubernetes_group_kind_version
                                    {
                                        let response_schema = definitions
                                            .get(&crate::swagger20::DefinitionPath(
                                                ref_path.path.clone(),
                                            ))
                                            .unwrap_or_else(|| {
                                                panic!(
                                                    "operation {} returns undefined type {:?}",
            ...
            if operation.kubernetes_action == Some(crate::swagger20::KubernetesAction::Put) {
                if let crate::swagger20::OperationResponses::Map(responses) = &operation.responses {
                    ...
                    if response_status_codes != [http::StatusCode::OK, http::StatusCode::CREATED]
                        && response_status_codes != [http::StatusCode::OK]
                    // 1.8 and earlier did not have 201
                    {
