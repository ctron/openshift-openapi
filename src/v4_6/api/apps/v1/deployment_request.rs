                ...
                A: serde::de::MapAccess<'de>,
            {
                ...
                while let Some(key) = serde::de::MapAccess::next_key::<Field>(&mut map)? {
                    match key {
                        ...
                        Field::Key_exclude_triggers => {
                            value_exclude_triggers = serde::de::MapAccess::next_value(&mut map)?
                        }
                        Field::Key_force => {
                            value_force = Some(serde::de::MapAccess::next_value(&mut map)?)
                        }
                        Field::Key_latest => {
                            value_latest = Some(serde::de::MapAccess::next_value(&mut map)?)
                        }
                        Field::Key_name => {
                ...
                Ok(DeploymentRequest {
                    ...
                    latest: value_latest
                        .ok_or_else(|| serde::de::Error::missing_field("latest"))?,
...
impl serde::Serialize for DeploymentRequest {
        ...
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct(
            <Self as k8s_openapi::Resource>::KIND,
            5 + self.exclude_triggers.as_ref().map_or(0, |_| 1),
        )?;
