...
impl UserIdentityMapping {
    ...
    pub fn delete_user_identity_mapping(
        ...
        let __url = format!("/apis/user.openshift.io/v1/useridentitymappings/{name}",
...
impl UserIdentityMapping {
    ...
    ) -> Result<(http::Request<Vec<u8>>, fn(http::StatusCode) -> k8s_openapi::ResponseBody<ReadUserIdentityMappingResponse>), k8s_openapi::RequestError> {
        let ReadUserIdentityMappingOptional {
            pretty,
        } = optional;
        let __url = format!("/apis/user.openshift.io/v1/useridentitymappings/{name}?",
            name = k8s_openapi::percent_encoding::percent_encode(name.as_bytes(), k8s_openapi::percent_encoding2::PATH_SEGMENT_ENCODE_SET),
